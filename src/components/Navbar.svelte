<script lang="ts">
	...
	let modal;
	import Rive from './Icons/rive.svelte';
	// Model Items
	import NavigationSection from './UI/Menu/NavigationSection.svelte';
	import Item from './UI/Menu/item.svelte';
	import PostSection from './UI/Menu/PostSection.svelte';
	import Post from './UI/Menu/post.svelte';
...
<header class="flex items-center w-full max-w-5xl px-4 pt-6 bg-transparent gap-x-4">
	<div id="Uni">
		<svg
			width="39"
			height="39"
		...
		>
			<path
				class="fill-white dark:fill-[#F4ADFF] stroke-[#0085FF] dark:stroke-[#7000FF] stroke-[4] transition-colors"
			...
			/>
			<path
				class="hidden dark:block fill-[#FFB0E9] stroke-[#FF00C7] stroke-[4] transition-colors"
			...
			/>
			<path
				class="stroke-2 stroke-white dark:stroke-[#2B004E] transition-colors"
	...
	<button
		on:click={() => modal.show()}
		class="
		hidden sm:flex flex-none flex-shrink-0 w-full sm:w-auto gap-x-2
		px-3 py-2 rounded-xl 
		text-white text-lg font-semibold leading-6 
		bg-blue-1000/30 hover:bg-blue-1000/50
		dark:bg-purple-700 dark:hover:bg-purple-500 
		focus:ring focus:outline-none focus:ring-offset-2 dark:focus:ring-offset-white dark:focus:ring-violet-500 
		duration-200 active:scale-105 ease-snappy transition-transform-colors 
		"
	>
		<span class=""> Open Menu </span>
		<span
			style="opacity: 1;"
			class="hidden sm:block text-gray-400 dark:text-purple-1000 text-sm leading-5 py-0.5 px-1.5 border border-slate-200 rounded-md"
		>
			<span class="sr-only">Press</span>
			<kbd class="inline font-jetMono">
				<abbr title="Control"> Ctrl </abbr>
			</kbd>
			<span class="sr-only"> and </span>
			<kbd class="inline font-jetMono">
				<abbr title="Slash"> &#8725; </abbr>
			</kbd>
			<span class="sr-only"> to Open Menu </span>
		</span>
	</button>
	<div class="w-full" />
	<Rive art="themeBtn" />
	<Rive art="nav" />
	<nav class="hidden sm:flex gap-x-4">
		<!-- Notes -->
		<a
			href="notes"
			class="p-2 text-white transition-colors duration-200 rounded-md hover:bg-slate-700/40 dark:hover:bg-pink-100 dark:text-purple-600"
		>
			<svg
				width="20"
				height="24"
			...
			>
				<path
					d="M13.6471 22.5H5.21569H3C1.89543 22.5 1 21.6046 1 20.5V13.0021C1 12.3495 1.31842 11.7379 1.85307 11.3636L2.06876 11.2126C3.39432 10.2848 5.21569 11.2331 5.21569 12.8511V15.2006C5.21569 17.031 7.47072 17.8997 8.69876 16.5424L9.93253 15.1788C11.1354 13.8493 13.3452 14.6521 13.4141 16.4438L13.6471 22.5ZM13.6471 22.5H16.0517C17.1563 22.5 18.0517 21.6046 18.0517 20.5V3C18.0517 1.89543 17.1563 1 16.0517 1H13.2255M1 8.71034V3C1 1.89543 1.89543 1 3 1H13.2255M13.2255 1V12.3824"
					stroke="currentColor"
					stroke-width="2"
...
<Modal bind:this={modal}>
	<!-- <span class="close" on:click={() => modal.hide()}>&times;</span> -->
	<section class="grid grid-cols-1 sm:grid-cols-2 place-content-center gap-x-8">
		<ul class="flex flex-col justify-center gap-y-6 sm:gap-y-12">
			<NavigationSection title="Navigation">
				{#each [...links] as [ name, link ] }
					<Item fun={modal.hide} href={`/${link}`} label={name} >
						<Rive art={name} />
					</Item>
				{/each}
			</NavigationSection>
